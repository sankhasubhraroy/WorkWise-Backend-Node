...
exports.validateFreelancer = async (req, res) => {
  try {
    const freelancer = await Freelancer.findById(req.user.id).select(
...
exports.register = async (req, res) => {
  try {
    ...
    const username = await generateUsername(Freelancer, name);
...
exports.googleCallback = async (req, res) => {
  try {
    ...
    let freelancer = await Freelancer.findOne({ email: data.email }).select(
      "-password"
    );
...
exports.forgetPassword = async (req, res) => {
  try {
    ...
    console.log(
      "is Success:",
      await sendMail(freelancer.email, "Reset Password", content)
    );
